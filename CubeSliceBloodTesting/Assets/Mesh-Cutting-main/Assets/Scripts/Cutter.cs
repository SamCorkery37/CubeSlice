...
public static class Cutter
{
    ...
    public static void Cut(GameObject originalGameObject, Vector3 contactPoint, Vector3 cutNormal)
    {
        ...
        try
        {
            ...
            // Update the original GameObject (left half)
            originalGameObject.GetComponent<MeshFilter>().mesh = finishedLeftMesh;

            // Recreate the collider for the left half
            ...
            // Apply the original object's material to the left half (cut)
            ApplyMaterials(originalGameObject, finishedLeftMesh.subMeshCount, GetCrossSectionMaterial(originalGameObject));

            // **POOLING**: Spawn the right half from the pool
            GameObject right = SlicedObjectPooler.Instance.SpawnFromPool("SlicedObject", originalGameObject.transform.position, originalGameObject.transform.rotation);

            // **Ensure the local scale is the same as the original object**
            right.transform.localScale = originalGameObject.transform.localScale;

            // Assign dynamically generated mesh to the pooled object
            right.GetComponent<MeshFilter>().mesh = finishedRightMesh;

            // Update the collider for the right half
            var rightCollider = right.GetComponent<MeshCollider>();
            if (rightCollider != null)
            {
                Object.Destroy(rightCollider);  // Destroy any existing collider before assigning a new one
            }
            rightCollider = right.AddComponent<MeshCollider>();
            rightCollider.sharedMesh = finishedRightMesh;
            rightCollider.convex = true;

            // Apply materials to the right half
            ApplyMaterials(right, finishedRightMesh.subMeshCount, GetCrossSectionMaterial(originalGameObject));

            // Add Rigidbody and force to the right half
            var rightRigidbody = right.GetComponent<Rigidbody>();
            rightRigidbody.AddRelativeForce(-cutPlane.normal * 250f);

            // Optional: Return the right half to the pool after a certain time
            SlicedObjectPooler.Instance.StartCoroutine(SlicedObjectPooler.Instance.ReturnToPool(right, 5f)); // Adjust delay as needed
        }
